...
async def websocket_tunnel(websocket: WebSocket, uuid: str):
    ...
    try:
        ...
        # ارتباط با سایت مقصد برقرار می‌شود
        reader, writer = await asyncio.wait_for(asyncio.open_connection(address, port), timeout=10.0)
        
        # === هسته اصلی رفع باگ: ارسال بلافاصله تاییدیه (0x00 0x00) ===
        ...
        if conn_id:
            ...
            if info:
                uid_info = info.get("uuid")
                ip_info = info.get("ip")
                if uid_info and ip_info:
                    has_other = any(c.get("uuid") == uid_info and c.get("ip") == ip_info for c in connections.values())
                    if not has_other:
                        remove_ip_from_link(uid_info, ip_info)
