...
class Config:
    ...
    SECRET_KEY: str = os.environ.get("SECRET_KEY", "ren-default-secret-key")
    ADMIN_PASSWORD: str = os.environ.get("ADMIN_PASSWORD", "admin")
    SESSION_COOKIE: str = "ren_session"
    SESSION_TTL: int = 60 * 60 * 24 * 7
    RELAY_BUF: int = 64 * 1024
...
class AppState:
    def __init__(self):
        self.auth = {"password_hash": hash_password(Config.ADMIN_PASSWORD)}
        ...
        self.link_ip_map: Dict[str, Set[str]] = defaultdict(set)
        ...
        self.error_logs: deque = deque(maxlen=50)
        self.hourly_traffic: Dict[str, int] = defaultdict(int)
        
        self.custom_addresses: List[str] = ["www.speedtest.net"]
        self.custom_addresses_lock = asyncio.Lock()
        
        self.custom_domain: str = ""
        self.custom_domain_lock = asyncio.Lock()
...
# ==========================================
def get_domain() -> str:
    domain = os.environ.get("RENDER_EXTERNAL_URL", os.environ.get("RAILWAY_PUBLIC_DOMAIN", "localhost"))
...
def generate_vless_link(uuid: str, remark: str = "REN", address: Optional[str] = None) -> str:
    domain = state.custom_domain if state.custom_domain else get_domain()
    addr = address if address else domain
    params = {
        "encryption": "none", "security": "tls", "type": "ws",
        "host": domain, "path": f"/ws/{uuid}", "sni": domain,
...
def compute_expiry(expiry_days: float) -> str:
...
def expiry_epoch(link: Dict) -> int:
    exp = link.get("expiry")
    if not exp: return 0
    try:
...
async def add_usage(uid: str, size: int):
    async with state.links_lock:
        if uid in state.links:
            state.links[uid]["used_bytes"] += size

async def relay_stream(reader, writer, is_ws_to_tcp, websocket, conn_id, link_uid):
    first_packet = True
    try:
        while True:
            if is_ws_to_tcp:
                ...
                data = await reader.read(Config.RELAY_BUF)
            ...
            if is_ws_to_tcp: state.stats["total_requests"] += 1
            ...
            if is_ws_to_tcp:
                ...
                await websocket.send_bytes((b"\x00\x00" + data) if first_packet else data)
                first_packet = False
    ...
    try:
        async with state.links_lock:
            ...
            if not link or not link["active"]:
                return await websocket.close(code=1008, reason="link disabled")
            if is_expired(link):
                return await websocket.close(code=1008, reason="link expired")
                
            max_conn = link.get("max_connections", 0)
            if max_conn > 0 and client_ip not in state.link_ip_map.get(uuid, set()):
                if len(state.link_ip_map.get(uuid, set())) >= max_conn:
                    return await websocket.close(code=1008, reason="connection limit reached")

        first_msg = await asyncio.wait_for(websocket.receive(), timeout=15.0)
        first_chunk = first_msg.get("bytes") or (first_msg.get("text") or "").encode()
        if not first_chunk or first_msg["type"] == "websocket.disconnect": return
        
        command, address, port, initial_payload = await parse_vless_header(first_chunk)
        
        conn_id = secrets.token_urlsafe(8)
        state.connections[conn_id] = {"uuid": uuid, "ip": client_ip, "connected_at": datetime.now().isoformat(), "bytes": 0}
        ...
        if initial_payload:
            ...
            state.connections[conn_id]["bytes"] += p_size
            state.hourly_traffic[datetime.now().strftime("%H:00")] += p_size
            await add_usage(uuid, p_size)
            writer.write(initial_payload)
            await writer.drain()

        task_up = asyncio.create_task(relay_stream(None, writer, True, websocket, conn_id, uuid))
        task_down = asyncio.create_task(relay_stream(reader, None, False, websocket, conn_id, uuid))
        ...
        if conn_id:
            ...
            if info:
                ...
                if uid and ip and not any(c.get("uuid") == uid and c.get("ip") == ip for c in state.connections.values()):
                    if uid in state.link_ip_map:
                        state.link_ip_map[uid].discard(ip)

# ==========================================
# 5. API Routes & Dependencies
# ==========================================
...
async def startup():
    ...
    logger.info(f"REN Gateway started on port {Config.PORT}")
...
async def api_login(request: Request):
    ...
    if hash_password(password) != state.auth["password_hash"]:
    ...
    resp.set_cookie(key=Config.SESSION_COOKIE, value=token, max_age=Config.SESSION_TTL, httponly=True, samesite="lax", path="/")
    return resp
...
async def api_change_password(request: Request):
    ...
    if hash_password(current_password) != state.auth["password_hash"]:
        raise HTTPException(status_code=400, detail="Current password is incorrect")
    if len(new_password) < 4:
        raise HTTPException(status_code=400, detail="Password must be at least 4 characters")
        
    state.auth["password_hash"] = hash_password(new_password)
    current_token = request.cookies.get(Config.SESSION_COOKIE)
    
    async with state.sessions_lock:
        state.sessions.clear()
        if current_token:
            state.sessions[current_token] = time.time() + Config.SESSION_TTL
    return {"ok": True}
...
async def get_stats():
    return {
        ...
        "hourly_traffic": dict(state.hourly_traffic),
    }

@app.post("/api/links", dependencies=[Depends(require_auth)])
async def create_link(request: Request):
    ...
    if not re.match(r'^[a-zA-Z0-9\-_. ]+$', label):
        raise HTTPException(status_code=400, detail="Invalid inbound name format")
        
    async with state.links_lock:
        if label in state.links:
            raise HTTPException(status_code=400, detail="Name already exists")
            
        limit_value = float(body.get("limit_value", 0.0))
        limit_unit = str(body.get("limit_unit", "GB"))
        limit_bytes = 0 if limit_value <= 0 else parse_size_to_bytes(limit_value, limit_unit)
        max_conn = int(body.get("max_connections", 0))
        expiry_days = float(body.get("expiry_days", 0.0))
        
        uid = label
        state.links[uid] = {
            "label": label, "limit_bytes": limit_bytes, "used_bytes": 0,
            "max_connections": max(0, max_conn),
            "created_at": datetime.now().isoformat(),
            "active": True, "expiry": compute_expiry(expiry_days)
        }
        
    return {"uuid": uid, "vless_link": generate_vless_link(uid, remark=f"REN-{label}")}
    ...
    async with state.links_lock:
        for uid, data in state.links.items():
            result.append({
                ...
                "current_connections": len(state.link_ip_map.get(uid, set())),
...
@app.patch("/api/links/{uid}", dependencies=[Depends(require_auth)])
async def update_link(uid: str, request: Request):
    ...
    async with state.links_lock:
        ...
        if "expiry_days" in body: link["expiry"] = compute_expiry(float(body["expiry_days"]))
        if body.get("label"): link["label"] = str(body["label"])[:60]
        if "max_connections" in body: link["max_connections"] = max(0, int(body["max_connections"]))
        
    return {"ok": True}

@app.delete("/api/links/{uid}", dependencies=[Depends(require_auth)])
...
@app.get("/api/domain", dependencies=[Depends(require_auth)])
async def get_custom_domain():
    async with state.custom_domain_lock:
        return {"domain": state.custom_domain}

@app.post("/api/domain", dependencies=[Depends(require_auth)])
async def set_custom_domain(request: Request):
    try: body = await request.json()
    except: body = {}
    ...
    return {"ok": True, "domain": state.custom_domain}

@app.get("/api/addresses", dependencies=[Depends(require_auth)])
async def list_addresses():
    async with state.custom_addresses_lock:
        return {"addresses": list(state.custom_addresses)}

@app.post("/api/addresses", dependencies=[Depends(require_auth)])
async def add_address(request: Request):
    ...
    async with state.custom_addresses_lock:
        if address in state.custom_addresses:
            raise HTTPException(status_code=400, detail="Address already exists")
        state.custom_addresses.append(address)
    return {"ok": True, "addresses": list(state.custom_addresses)}

@app.delete("/api/addresses/{index}", dependencies=[Depends(require_auth)])
async def delete_address(index: int):
    async with state.custom_addresses_lock:
        if 0 <= index < len(state.custom_addresses):
            state.custom_addresses.pop(index)
        else:
            raise HTTPException(status_code=404, detail="Address not found")
    return {"ok": True, "addresses": list(state.custom_addresses)}

@app.get("/api/links/{uid}/sub", dependencies=[Depends(require_auth)])
...
{vless_link}"""
    ...
    return {
        "subscription_url": f"{get_domain()}/sub/{uid}",
        "config": vless_link,
        "label": link["label"],
        "used_bytes": used,
        "limit_bytes": limit,
...
async def subscription_endpoint(uid: str):
    ...
    async with state.custom_addresses_lock:
        addresses = list(state.custom_addresses)
