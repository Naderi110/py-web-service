...
logger = logging.getLogger("Traffic-Router")

app = FastAPI(title="NetBridge", docs_url=None, redoc_url=None)
...
APP_CFG = {
    ...
    "enc_key": os.environ.get("SECRET_KEY", "default-secure-key-123"),
    "panel_pass": os.environ.get("ADMIN_PASSWORD", "admin")
}
...
class RouterState:
    def __init__(self):
        self.live_streams: Dict[str, Dict[str, Any]] = {}
        self.stream_sockets: Dict[str, WebSocket] = {}
        self.user_ip_log: Dict[str, Set[str]] = defaultdict(set)
        
        self.users: Dict[str, Dict[str, Any]] = {}
        self.users_lock = asyncio.Lock()
        
        self.sys_metrics = {"xfer_bytes": 0, "reqs": 0, "errs": 0, "boot_time": time.time()}
        self.fault_log: deque = deque(maxlen=50)
        self.hourly_graph: Dict[str, int] = defaultdict(int)
        
        self.allowed_ips: List[str] = ["www.speedtest.net"]
        self.allowed_lock = asyncio.Lock()
...
core_state = RouterState()

def secure_hash(plain: str) -> str:
    return hashlib.sha256(f"{plain}{APP_CFG['enc_key']}".encode()).hexdigest()

auth_db = {"master_hash": secure_hash(APP_CFG["panel_pass"])}

def extract_base_host() -> str:
    dom = os.environ.get("RENDER_EXTERNAL_URL", os.environ.get("RAILWAY_PUBLIC_DOMAIN", "localhost"))
    return dom.replace("https://", "").replace("http://", "")

def make_access_uri(uid: str, tag: str = "REN", spec_ip: str = None) -> str:
    base = core_state.alias_domain if core_state.alias_domain else extract_base_host()
    target = spec_ip if spec_ip else base
    query_map = {
        "encryption": "none", "security": "tls", "type": "ws",
        "host": base, "path": f"/ws/{quote(uid)}", "sni": base,
        "fp": "chrome", "alpn": "http/1.1",
    }
    encoded = "&".join(f"{k}={quote(str(v))}" for k, v in query_map.items())
    return f"vless://{uid}@{target}:443?{encoded}#{quote(tag)}"

def get_uptime_str() -> str:
    secs = int(time.time() - core_state.sys_metrics["boot_time"])
    h, m, s = secs // 3600, (secs % 3600) // 60, secs % 60
    return f"{h:02d}:{m:02d}:{s:02d}"

def calculate_bytes(val: float, unit: str) -> int:
    mul = {"GB": 1024**3, "MB": 1024**2, "KB": 1024}.get(unit.upper(), 1)
    return int(val * mul)

def get_future_date(days: float) -> str:
    return (datetime.now() + timedelta(days=days)).isoformat() if days > 0 else ""

def is_user_expired(data: dict) -> bool:
    exp = data.get("expiry")
    if not exp: return False
    try: return datetime.now() >= datetime.fromisoformat(exp)
    except Exception: return False

def get_expire_timestamp(data: dict) -> int:
    exp = data.get("expiry")
    if not exp: return 0
...
# ==========================================
# Network Bridge & Tunnel Engine
# ==========================================
async def decode_packet_header(initial_data: bytes):
    try:
        if len(initial_data) < 24: return None, None, None, None
        idx = 17
        ext_size = initial_data[idx]
        idx += 1 + ext_size
        cmd_code = initial_data[idx]
        idx += 1
        dest_port = int.from_bytes(initial_data[idx:idx + 2], "big")
        idx += 2
        addr_flag = initial_data[idx]
        idx += 1
        
        if addr_flag == 1:
            dest_ip = ".".join(str(b) for b in initial_data[idx:idx + 4])
            idx += 4
        elif addr_flag == 2:
            dom_len = initial_data[idx]
            idx += 1
            dest_ip = initial_data[idx:idx + dom_len].decode("utf-8", errors="ignore")
            idx += dom_len
        elif addr_flag == 3:
            ip6_bytes = initial_data[idx:idx + 16]
            dest_ip = ":".join(f"{ip6_bytes[i]:02x}{ip6_bytes[i+1]:02x}" for i in range(0, 16, 2))
            idx += 16
        else:
            return None, None, None, None
            
        return cmd_code, dest_ip, dest_port, initial_data[idx:]
    except Exception:
        return None, None, None, None

async def verify_allowance(uid: str, data_size: int) -> bool:
    async with core_state.users_lock:
        usr = core_state.users.get(uid)
        if not usr or not usr["active"] or is_user_expired(usr): return False
        if usr["limit_bytes"] == 0: return True
        return (usr["used_bytes"] + data_size) <= usr["limit_bytes"]

async def log_traffic(uid: str, size: int):
    async with core_state.users_lock:
        if uid in core_state.users: core_state.users[uid]["used_bytes"] += size

async def stream_bridge(reader, writer, is_inbound, websocket, session_id, profile_id):
    try:
        while True:
            if is_inbound:
            ...
            if not await verify_allowance(profile_id, length):
                await websocket.close(code=1008)
                break
                
            core_state.sys_metrics["xfer_bytes"] += length
            if is_inbound: core_state.sys_metrics["reqs"] += 1
            core_state.active_sessions[session_id]["bytes"] += length
            core_state.hourly_graph[datetime.now().strftime("%H:00")] += length
            await log_traffic(profile_id, length)
...
async def handle_ws_tunnel(websocket: WebSocket, client_id: str):
    await websocket.accept()
    dest_writer = None
    sess_id = None
    
    caller_ip = websocket.headers.get("x-forwarded-for", "").split(",")[0].strip()
    if not caller_ip and websocket.client: caller_ip = websocket.client.host
        
    try:
        async with core_state.users_lock:
            usr = core_state.users.get(client_id)
            if not usr or not usr["active"] or is_user_expired(usr):
                return await websocket.close(code=1008)
            
            conn_limit = usr.get("max_connections", 0)
            if conn_limit > 0 and caller_ip not in core_state.user_ip_log.get(client_id, set()):
                if len(core_state.user_ip_log.get(client_id, set())) >= conn_limit:
                    return await websocket.close(code=1008)

        init_packet = await asyncio.wait_for(websocket.receive(), timeout=15.0)
        if init_packet["type"] == "websocket.disconnect": return
        payload_bytes = init_packet.get("bytes") or (init_packet.get("text") or "").encode()
        if not payload_bytes: return
        
        action, target_ip, target_port, remaining_data = await decode_packet_header(payload_bytes)
        if action is None: return await websocket.close(code=1003)
        
        sess_id = secrets.token_urlsafe(8)
        core_state.active_sessions[sess_id] = {"uuid": client_id, "ip": caller_ip, "bytes": 0}
        core_state.stream_sockets[sess_id] = websocket
        core_state.user_ip_log[client_id].add(caller_ip)
        
        chunk_len = len(payload_bytes)
        core_state.sys_metrics["xfer_bytes"] += chunk_len
        core_state.sys_metrics["reqs"] += 1
        await log_traffic(client_id, chunk_len)

        dest_reader, dest_writer = await asyncio.wait_for(asyncio.open_connection(target_ip, target_port), timeout=10.0)
        
        # <<< اینجا هندل‌شیک بلافاصله به کلاینت فرستاده می‌شود تا قفل نکند >>>
        await websocket.send_bytes(b"\x00\x00")
        
        if remaining_data:
            dest_writer.write(remaining_data)
            await dest_writer.drain()
            core_state.sys_metrics["xfer_bytes"] += len(remaining_data)
            await log_traffic(client_id, len(remaining_data))

        t1 = asyncio.create_task(stream_bridge(None, dest_writer, True, websocket, sess_id, client_id))
        t2 = asyncio.create_task(stream_bridge(dest_reader, None, False, websocket, sess_id, client_id))
        
        done, pending = await asyncio.wait({t1, t2}, return_when=asyncio.FIRST_COMPLETED)
        for t in pending: t.cancel()
        
    except Exception as e:
        core_state.sys_metrics["errs"] += 1
    finally:
        if dest_writer:
            try: dest_writer.close()
            except: pass
        if sess_id:
            info = core_state.active_sessions.pop(sess_id, None)
            core_state.stream_sockets.pop(sess_id, None)
            if info:
                uid, ip = info.get("uuid"), info.get("ip")
                if uid and ip and not any(c.get("uuid") == uid and c.get("ip") == ip for c in core_state.active_sessions.values()):
                    core_state.user_ip_log[uid].discard(ip)

# ==========================================
# Control Panel APIs
# ==========================================
async def check_access(request: Request):
    tkn = request.cookies.get("ren_session")
    if not tkn: raise HTTPException(status_code=401)
    async with core_state.session_lock:
        exp = core_state.active_sessions.get(tkn)
        if exp is None or exp < time.time():
            core_state.active_sessions.pop(tkn, None)
            raise HTTPException(status_code=401)
    return tkn

@app.on_event("startup")
async def on_boot():
    core_state.web_client = httpx.AsyncClient(timeout=10.0)
    async with core_state.users_lock:
        if not core_state.users:
            core_state.users["Default"] = {"label": "Default", "limit_bytes": 0, "used_bytes": 0, "max_connections": 0, "created_at": datetime.now().isoformat(), "active": True, "expiry": ""}

@app.on_event("shutdown")
async def on_stop():
    if core_state.web_client: await core_state.web_client.aclose()

@app.post("/api/login")
async def process_login(request: Request):
    try: req_data = await request.json()
    except: req_data = {}
    if secure_hash(str(req_data.get("password", ""))) != auth_db["master_hash"]:
        raise HTTPException(status_code=401, detail="Invalid password")
    tkn = secrets.token_urlsafe(32)
    async with core_state.session_lock:
        core_state.active_sessions[tkn] = time.time() + 604800
    res = JSONResponse({"ok": True})
    res.set_cookie("ren_session", tkn, max_age=604800, httponly=True, path="/")
    return res

@app.post("/api/logout")
async def process_logout(request: Request):
    tkn = request.cookies.get("ren_session")
    async with core_state.session_lock: core_state.active_sessions.pop(tkn, None)
    res = JSONResponse({"ok": True})
    res.delete_cookie("ren_session", path="/")
    return res

@app.get("/stats")
async def get_system_stats(_=Depends(check_access)):
    return {
        "active_connections": len(core_state.active_sessions),
        "total_traffic_mb": round(core_state.sys_metrics["xfer_bytes"] / (1024**2), 2),
        "total_requests": core_state.sys_metrics["reqs"],
        "total_errors": core_state.sys_metrics["errs"],
        "uptime": get_uptime_str(),
        "links_count": len(core_state.users),
        "domain": extract_base_host(),
        "cpu_percent": psutil.cpu_percent(interval=0.1),
        "memory_percent": psutil.virtual_memory().percent,
        "hourly_traffic": dict(core_state.hourly_graph)
    }

@app.post("/api/links")
async def create_user(request: Request, _=Depends(check_access)):
    try: req = await request.json()
    except: req = {}
    lbl = str(req.get("label", "")).strip()[:60]
    if not re.match(r'^[a-zA-Z0-9\-_. ]+$', lbl): raise HTTPException(400, "Invalid format")
    async with core_state.users_lock:
        if lbl in core_state.users: raise HTTPException(400, "Name exists")
        limit = float(req.get("limit_value", 0))
        max_conn = max(0, int(req.get("max_connections", 0)))
        core_state.users[lbl] = {
            "label": lbl, "limit_bytes": calculate_bytes(limit, req.get("limit_unit", "GB")), 
            "used_bytes": 0, "max_connections": max_conn,
            "created_at": datetime.now().isoformat(), "active": True, 
            "expiry": get_future_date(float(req.get("expiry_days", 0)))
        }
    return {"uuid": lbl, "vless_link": make_access_uri(lbl, remark=f"REN-{lbl}")}

@app.get("/api/links")
async def list_users(_=Depends(check_access)):
    results = []
    async with core_state.users_lock:
        for uid, data in core_state.users.items():
            results.append({
                ...
                "expired": is_user_expired(data), "created_at": data.get("created_at", ""),
                "current_connections": len(core_state.user_ip_log.get(uid, set())), 
                "vless_link": make_access_uri(uid, remark=f"REN-{data['label']}")
            })
    return {"links": sorted(results, key=lambda x: x.get("created_at", ""), reverse=True)}

@app.patch("/api/links/{uid}")
async def update_user(uid: str, request: Request, _=Depends(check_access)):
    try: req = await request.json()
    except: req = {}
    async with core_state.users_lock:
        if uid not in core_state.users: raise HTTPException(404)
        usr = core_state.users[uid]
        if "active" in req: usr["active"] = bool(req["active"])
        if "reset_usage" in req and req["reset_usage"]: usr["used_bytes"] = 0
        if "limit_value" in req: usr["limit_bytes"] = calculate_bytes(float(req["limit_value"]), req.get("limit_unit", "GB"))
        if "max_connections" in req: usr["max_connections"] = max(0, int(req["max_connections"]))
        if "expiry_days" in req: usr["expiry"] = get_future_date(float(req["expiry_days"]))
    return {"ok": True}

@app.delete("/api/links/{uid}")
async def remove_user(uid: str, _=Depends(check_access)):
    async with core_state.users_lock: core_state.users.pop(uid, None)
    return {"ok": True}

@app.get("/api/domain")
async def fetch_domain(_=Depends(check_access)):
    async with core_state.alias_lock: return {"domain": core_state.alias_domain}

@app.post("/api/domain")
async def set_domain(request: Request, _=Depends(check_access)):
    try: req = await request.json()
    except: req = {}
    async with core_state.alias_lock:
        core_state.alias_domain = str(req.get("domain", "")).strip().replace("https://", "").replace("http://", "").rstrip("/")
    return {"ok": True}

@app.get("/api/addresses")
async def fetch_addresses(_=Depends(check_access)):
    async with core_state.allowed_lock: return {"addresses": list(core_state.allowed_ips)}

@app.post("/api/addresses")
async def add_address(request: Request, _=Depends(check_access)):
    try: req = await request.json()
    except: req = {}
    addr = str(req.get("address", "")).strip()
    if addr:
        async with core_state.allowed_lock:
            if addr not in core_state.allowed_ips: core_state.allowed_ips.append(addr)
    return {"ok": True}

@app.delete("/api/addresses/{idx}")
async def remove_address(idx: int, _=Depends(check_access)):
    async with core_state.allowed_lock:
        if 0 <= idx < len(core_state.allowed_ips): core_state.allowed_ips.pop(idx)
    return {"ok": True}

@app.get("/api/links/{uid}/sub", dependencies=[Depends(check_access)])
async def fetch_sub_data(uid: str):
    async with core_state.users_lock:
        usr = core_state.users.get(uid)
        if usr is None: raise HTTPException(404)
            
    access_str = make_access_uri(uid, remark=f"REN-{usr['label']}")
    used = usr["used_bytes"]
    limit = usr["limit_bytes"]
    ...
    import base64
    sub_content = f"# REN Subscription\n# Label: {usr['label']}\n# Used: {used_mb} MB / {limit_mb if limit > 0 else 'Unlimited'} MB\n# Remaining: {remaining_mb if limit > 0 else 'Unlimited'} MB\n# Usage: {pct}%\n# Status: {'Active' if usr['active'] else 'Disabled'}\n# Expiry: {usr.get('expiry', '')[:10] if usr.get('expiry') else 'Unlimited'}\n{access_str}"
    encoded = base64.b64encode(sub_content.encode()).decode()
    
    return {
        "subscription_url": f"https://{extract_base_host()}/sub/{uid}",
        "config": access_str,
        "label": usr["label"],
        "used_bytes": used,
        "limit_bytes": limit,
...
@app.get("/sub/{uid}")
async def sub_generator(uid: str):
    import base64
    async with core_state.users_lock:
        usr = core_state.users.get(uid)
        if not usr or not usr["active"] or is_user_expired(usr): raise HTTPException(403)
    async with core_state.allowed_lock: ips = list(core_state.allowed_ips)
    lines = [make_access_uri(uid, remark=f"REN-{usr['label']}-Main")]
    for i, ip in enumerate(ips): lines.append(make_access_uri(uid, remark=f"REN-{usr['label']}-IP{i+1}", spec_ip=ip))
    encoded = base64.b64encode("\n".join(lines).encode()).decode()
    return Response(content=encoded, headers={"Content-Type": "text/plain; charset=utf-8", "profile-update-interval": "6", "subscription-userinfo": f"upload={usr['used_bytes']}; download=0; total={usr['limit_bytes']}; expire={get_expire_timestamp(usr)}"})

@app.post("/api/change-password")
async def update_security(request: Request, _=Depends(check_access)):
    try: req = await request.json()
    except: req = {}
    if secure_hash(str(req.get("current_password", ""))) != auth_db["master_hash"]: raise HTTPException(400)
    auth_db["master_hash"] = secure_hash(str(req.get("new_password", "")))
    return {"ok": True}

# ==========================================
# Frontend HTML Strings
# ==========================================
...
function updateChart(){
  if(!trafficChart||!core_state.hourly_graph)return;
  const ht=core_state.hourly_graph;
...
async def serve_login(request: Request):
    ...
    if tkn:
        async with core_state.session_lock:
            if tkn in core_state.active_sessions and core_state.active_sessions[tkn] > time.time():
...
if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=APP_CFG["server_port"])